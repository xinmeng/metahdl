...
extern set<string> NO_SANITY_CHECK;
...
private:
  string _name;
  bool _need_sanity_check;
  bool _fsm_nc;
  CSymbol *_clk, *_rst;
  map<string, CStTransition*> *_graph;
  string _unreachable_state, _dead_state;
...
public:
  inline CBlkFSM(const yy::location &loc, 
    ...
    _need_sanity_check (true), _unreachable_state (""), _dead_state ("") {}
...
//     _ff (NULL), _init (NULL), _body (NULL) {}
  ...
  inline void Print(ostream&os=cout) {
    if (_need_sanity_check) {
        os << "`ifdef RTL_ASSERTION_ON" << endl
           << "  " << _name << "_cs_sanity_check: assert property (@(fsm_sanity_check) " << _name << "_cs == "
           << (*_states)[0]->Name() << ")" << endl
           << "    else `uve_fatal(sva_log, $psprintf(\"FSM state in %b\", " << _name << "_cs));" << endl
           << "`endif" << endl
           << endl;
    }
    else {
        os << "// fsm_sanity_check is not added due to: " << endl
           << "// Unreachable state: " << _unreachable_state << endl
           << "// Dead state: " << _dead_state << endl
           << endl;
    }    
  ...
  inline void CheckFSM() {
	...
	str = str + "State \"" + iter->first + "\" is unreachable. ";
        _unreachable_state = _unreachable_state + " " + iter->first;
      }
      if ( iter->second->to.size() == 0 ) {
	has_err = true;
	str = str + "State \"" + iter->first + "\" is dead state. ";
        _dead_state = _dead_state + " " + iter->first;
      }
    }

    if ( has_err ) {
        _need_sanity_check = false;
        str = str + "fsm_sanity_check assertion will not be added.";
        throw str;
    }

    if (NO_SANITY_CHECK.count(_name))
        _need_sanity_check = false;
  }
